        ...
        <div className="min-h-screen bg-[#FCF9F2] font-body text-[#223322] flex flex-col">
            ...
            >
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                    ...
                    <nav className="hidden md:flex items-center gap-8 font-medium">
                        <Link href="/" className="hover:text-[#438240] transition-colors">Home</Link>
                        <Link href="/menu" className="hover:text-[#438240] transition-colors">Menu</Link>
                        <Link href="/about" className="hover:text-[#438240] transition-colors">About</Link>
                        <Link href="/contact" className="hover:text-[#438240] transition-colors">Contact</Link>
                    </nav>
                        ...
                        {user ? (
                            <div className="flex items-center gap-4">
                                <span className="text-sm font-medium text-gray-500">
                                    Hai, <span className="text-[#223322] font-bold">{getFirstName(user.name)}</span>
                                </span>
                                ...
                                {/* Dropdown Menu User (Burger Icon) */}
                                <div className="relative" ref={dropdownRef}>
                                    <button 
                                        onClick={() => setDesktopUserMenuOpen(!desktopUserMenuOpen)}
                                        className={`p-2.5 border rounded-full transition-colors focus:outline-none flex items-center justify-center ${
                                            desktopUserMenuOpen 
                                            ? 'bg-[#438240] text-white border-[#438240]' 
                                            : 'bg-white border-gray-200 text-[#223322] hover:bg-gray-50'
                                        }`}
                                        title="Menu Akun"
                                    >
                                        <Menu size={18} />
                            ...
                            <Link href="/login" className="bg-[#438240] text-white px-6 py-2.5 rounded-full font-bold shadow-lg shadow-[#438240]/30 hover:bg-[#366B33] hover:-translate-y-0.5 transition-all">
                                Login
                            </Link>
                        )}
                    </div>
            ...
            <AnimatePresence>
                {mobileMenuOpen && (
                    ...
                    >
                        <Link href="/" className="text-lg font-medium text-[#223322]">Home</Link>
                        <Link href="/menu" className="text-lg font-medium text-[#223322]">Menu</Link>
                        <hr className="border-gray-100" />
                        
                        {user ? (
                            <div className="flex flex-col gap-3">
                                <span className="text-center text-sm text-gray-500 mb-2">
                                    Masuk sebagai <span className="font-bold text-[#223322]">{user.name}</span>
                                </span>
                            ...
                            <Link href="/login" className="bg-[#438240] text-white text-center py-3 rounded-2xl font-bold mt-2 hover:bg-[#366B33] transition-colors">
                                Login
                            </Link>
                        )}
                    </motion.div>
            ...
            <footer className="bg-[#223322] text-[#FCF9F2] py-12 rounded-t-[3rem] mt-auto">
                <div className="max-w-7xl mx-auto px-6 text-center">
                    <img src="/images/logo-nyamaw.png" alt="Logo" className="h-14 mx-auto mb-4 brightness-0 invert opacity-90" />
                    <p className="text-[#8DA38D] mb-6 text-sm">Pesan makanan rumahan tanpa ribet.</p>
                    <div className="flex justify-center gap-6 text-sm font-medium mb-8 text-[#B4C4B4]">
                        <Link href="/menu" className="hover:text-white transition-colors">Menu</Link>
                        <Link href="/faq" className="hover:text-white transition-colors">FAQ</Link>
                        <Link href="/terms" className="hover:text-white transition-colors">Syarat & Ketentuan</Link>
                    </div>
                    <p className="text-[#597359] text-xs">&copy; 2026 Nyam.Aw. All rights reserved.</p>
                </div>
            </footer>
        </div>
    );
